            ...
            <div className=' w-full py-4 d-flex w-100  justify-content-center align-items-center'>
                <div className="col-lg-5">
                    {!login ? <form action="" key="signup" onSubmit={handleSignup} className='p-5 user-form rounded'>
                        ...
                        <div className="show-div position-relative">
                            <input type={hide ? "password" : "text" } name='password' value={signup.password} onChange={(e) => setSignup({...signup, password:e.target.value})} id="sign-password" className=' mb-3 rounded-0 ' rows="1" placeholder='Your Password' required />
                            <i style={{right: '10px', top: '30%'}} onClick={() => setHide(!hide)} className={`text-white fa-solid ${hide? "fa-eye-slash": "fa-eye"} position-absolute`}></i>
                        </div>    
                        <p className="text-black text-bold">* Please Login After Sign Up</p>
                    ...
                    <form action="" key="signin" onSubmit={handleSignIn} className='p-5 user-form rounded'>
                        ...
                        <div className="show-div position-relative">
                            <input type={hide ? "password" : "text" } value={signin.password} onChange={(e) => setSignin({...signin, password:e.target.value})} id="sign-password" className=' mb-3 rounded-0 ' rows="1" placeholder='Your Password' required />
                            <i style={{right: '10px', top: '30%'}} onClick={() => setHide(!hide)} className={`text-white fa-solid ${hide? "fa-eye-slash": "fa-eye"} position-absolute`}></i>
                        </div>    
